...
public class UnionFind {
    private static PriorityQueue<UnionFindNode> pq = new PriorityQueue<>();
...
//        //2
    ...
    public static int solution(int n, int[][] costs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            pq.add(new UnionFindNode(start, end, cost));
        }

        for (int i = 0; i < n; i++) {
            UnionFindNode current = pq.poll();
...
class UnionFindNode implements Comparable<UnionFindNode> {
    int start;
    int end;
    int cost;

    UnionFindNode(int start, int end, int cost) {
        this.start = start;
        this.end = end;
        this.cost = cost;
    }
